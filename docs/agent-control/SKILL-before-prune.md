...
**How closely to imitate.** The previous spec is the pattern, not a
straitjacket. If this spec genuinely needs a section the last one had no use
for, add it and keep everything else identical. Do not drop a section because
this spec's version of it would be short.
...
- Commits and pushes are the owner's alone — publish issues, never commit.
