class Container {
  ...
  addChild(child) {
    ...
    // (patched for ESM: Button lives on window, set by button.js)
    if (window.Button && child instanceof window.Button) {
    ...
    requestAnimationFrame(() => {
      ...
      if (newWidth !== this.width || newHeight !== this.height) {
        ...
        this.children.forEach(child => {
          // (patched for ESM: Button is optional — this build ships Container only)
          if (window.Button && child instanceof window.Button && child.isNestedGlass && child.gl_refs.gl) {
  ...
  capturePageSnapshot() {
    ...
    html2canvas(document.body, {
      ...
      // ESM patch: the snapshot must show the page's FINAL layout. Framer
      // Motion keeps below-the-fold reveals at inline opacity:0 until they
      // scroll into view — force those to their revealed state in the clone,
      // otherwise glass panels far down the page refract an empty snapshot.
...
// ESM patch (vendored for this project; upstream is a plain <script>)
